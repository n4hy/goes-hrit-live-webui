...
echo "[1/8] Packages..."
apt-get update -y
apt-get install -y nginx python3 ffmpeg
...
systemctl restart goes-sse.service
systemctl enable --now goes-sse.service
systemctl enable --now update-goes-fd-web.timer

echo "[8/8] Set permissions for timelapse generation..."
# Allow www-data to run the timelapse script and write to timelapse dir
